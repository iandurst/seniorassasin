...
async function refreshVote(){
  try{
    const v = await api('/get-vote');
    if(v.status === 'open'){
      el('#voteInfo').textContent = `Voting OPEN — ends at ${new Date(v.endAt).toLocaleString()}. ${v.counts.stop} stop / ${v.counts.continue} continue. Threshold: ${v.threshold}.`;
    }else if(v.status === 'closed'){
      el('#voteInfo').textContent = `Voting CLOSED — Result: ${v.result?.stopped ? 'STOP' : 'CONTINUE'} (${v.counts.stop} stop / ${v.counts.continue} continue, threshold ${v.threshold}).`;
    }else{
      el('#voteInfo').textContent = 'No active vote.';
    }
  }catch(e){
    el('#voteInfo').textContent = 'Unable to load vote status.';
  }
}
el('#btnStartVote').addEventListener('click', async () => {
  if(!confirm('Open the 3-hour vote window and text links to all eligible players?')) return;
  try{
    await api('/start-vote', { method:'POST', headers: adminHeaders() });
    await refreshVote();
    alert('Vote window opened.');
  }catch(err){
    alert(err.data?.error || 'Failed to start vote');
  }
});
el('#btnCloseVote').addEventListener('click', async () => {
  try{
    await api('/close-vote', { method:'POST', headers: adminHeaders() });
    await refreshVote();
  }catch(err){
    alert(err.data?.error || 'Failed to close vote');
  }
});

const observer = new MutationObserver(() => {
  api('/get-state').then(()=>{ loadState(); refreshVote(); }).catch(()=>{});
});
