...
async function refreshVoteState(){
  try{
    const res = await api('/vote?mode=state');
    const s = res.state || { open:false };
    if(s.open){
      el('#voteState').textContent = `Vote is OPEN (Week ${s.week}). Closes at ${new Date(s.closesAt).toLocaleString()}.`;
    }else if(s.result){
      const pct = Math.round((s.result.stopPct||0)*100);
      el('#voteState').textContent = `Last result: stop=${s.result.stop}, continue=${s.result.cont}, eligible=${s.result.totalEligible}, stop%=${pct} → outcome=${s.result.outcome}`;
    }else{
      el('#voteState').textContent = 'Vote is CLOSED.';
    }
  }catch(e){
    el('#voteState').textContent = 'Unable to load vote state.';
  }
}

el('#openVote').addEventListener('click', async () => {
  try{
    await api('/vote', { method:'POST', headers: adminHeaders(), body: JSON.stringify({ action:'open' }) });
    await refreshVoteState();
    alert('Vote opened.');
  }catch(err){ alert(err.data?.error || 'Failed to open vote'); }
});
el('#closeVote').addEventListener('click', async () => {
  try{
    await api('/vote', { method:'POST', headers: adminHeaders(), body: JSON.stringify({ action:'close' }) });
    await refreshVoteState();
    alert('Vote closed.');
  }catch(err){ alert(err.data?.error || 'Failed to close vote'); }
});

// Auto-load state when admin panel opens
const observer = new MutationObserver(() => {
  api('/get-state').then(()=>{ loadState(); refreshVoteState(); }).catch(()=>{});
});
