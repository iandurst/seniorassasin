(async function(){
  try{
    ...
    el('#count').textContent = String(participants.length);
    ...
    leaderboard.slice(0,10).forEach((p, i) => {
      ...
      tr.innerHTML = `<td>#${i+1}</td><td>${p.firstName} ${p.lastName}</td><td>${p.eliminations}</td><td><span class="badge ${p.alive?'alive':'out'}">${p.alive?'Alive':'Out'}</span></td>`;
