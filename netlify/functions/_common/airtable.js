...
// Accept multiple env var names, trim whitespace
...
// Serialize arrays the way Airtable expects (fields[]=A&fields[]=B)
...
async function request(method, url, { params = {}, data = null } = {}) {
  ...
  if ((m === 'POST' || m === 'PATCH' || m === 'PUT') && data != null) {
    config.data = data
  }
  try {
    ...
    const atErr = err.response?.data?.error
    console.error('Airtable request error', {
      method: m, url: config.url, status,
      airtableError: atErr || null
    })
    throw err
  }
}
...
function base(table) {
  return {
    ...
    async destroy(ids) {
      const usp = new URLSearchParams()
      ;(Array.isArray(ids) ? ids : [ids]).forEach(id => usp.append('records[]', id))
