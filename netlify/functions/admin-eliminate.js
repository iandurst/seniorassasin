...
exports.handler = async (event) => {
  ...
  try {
    ...
    await base('Eliminations').create({ 'EliminatorId': eliminatorId, 'EliminatedId': eliminatedId, 'Timestamp': new Date().toISOString() })
    const er = await base('Players').find(eliminatorId); const current = er.get('Eliminations') || 0
    await base('Players').update([{ id: eliminatorId, fields: { 'Eliminations': current + 1 } }])
    return ok({ ok:true })
  } catch (e) { console.error(e); return bad('Error recording elimination', 500) }
}
