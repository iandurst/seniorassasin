...
exports.handler = async (event) => {
  ...
  try { const { id, alive } = JSON.parse(event.body||'{}'); if(!id) return bad('Missing id'); const res = await base('Players').update([{ id, fields: { 'Alive': !!alive } }]); return ok({ ok:true, id: res[0].id }) }
