...
exports.handler = async (event) => {
  ...
  try {
    const settings = await base('Settings').select().firstPage().catch((e)=>{ const code=e?.response?.status; if(code===404||code===422){ throw new Error('Missing Airtable table/field. Create a table named "Settings" with a field "WeekStart" (Date/Time).') } return [] })
