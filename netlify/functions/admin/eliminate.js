...
exports.handler = async (event) => {
  ...
  try {
    ...
    const elim = await base('Players').find(eliminatedId)
    const elimFields = { 'Alive': false }
    await base('Players').update([{ id: eliminatedId, fields: elimFields }])
    await base('Eliminations').create({
      'EliminatorId': eliminatorId,
      'EliminatedId': eliminatedId,
      'Timestamp': new Date().toISOString()
    })
    // increment eliminations count for eliminator
    ...
    console.error(e)
    return bad('Error recording elimination', 500)
  }
}
