...
exports.handler = async (event) => {
  ...
  try{
    ...
    for(const group of chunks(allElims,10)){ await base('Eliminations').destroy(group.map(r=>r.id)) }
    const votes = await base('Votes').select().all()
    for(const group of chunks(votes,10)){ await base('Votes').destroy(group.map(r=>r.id)) }
    return ok({ ok: true })
