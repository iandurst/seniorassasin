...
const { ok, initBlobs } = require('./util');
...
exports.handler = async (event) => {
  ...
  const prizePool = settings.prizePoolOverride != null ? settings.prizePoolOverride : (Number(settings.entryFee||0) * eliminated.length);
