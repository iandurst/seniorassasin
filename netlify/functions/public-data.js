...
exports.handler = async () => {
  try{
    ...
    await base('Players').select({ view:'Grid view' }).eachPage(async (records)=>{
      records.forEach(r=>players.push({ id:r.id, firstName:r.get('FirstName')||'', lastName:r.get('LastName')||'', phone:'hidden', verified:!!r.get('Verified'), alive:r.get('Alive')!==false, eliminations:r.get('Eliminations')||0 }))
    })
