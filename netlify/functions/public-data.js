...
exports.handler = async () => {
  try {
    ...
    const verifiedPlayers = allPlayers.filter(p => p.verified)

    const elimCount = (await base('Eliminations').select({ pageSize: 100, fields: ['ID'] }).all()).length
    const prizePool = elimCount * 5

    const topPlayer = verifiedPlayers.slice().sort((a,b)=> (b.eliminations||0)-(a.eliminations||0))[0] || null

    return ok({ players: verifiedPlayers, topPlayer, totalElims: elimCount, prizePool, pendingCount: allPlayers.length - verifiedPlayers.length })
  } catch (e) {
    console.error(e)
