...
exports.handler = async (event) => {
  ...
  try {
    ...
    const rec = await base('Players').create({ 'FirstName': firstName.trim(), 'LastName': lastName.trim(), 'Phone': phone, 'Verified': false, 'Alive': false, 'Eliminations': 0 })
    return ok({ id: rec.id, ok: true })
  } catch (e) { console.error(e); const code=e.response?.status; if(code===401) return bad('Airtable auth failed. Check AIRTABLE_API_KEY / BASE', 500); return bad('Error creating signup', 500) }
}
