...
async function sendSms(to, body){
  ...
  const from = process.env.TELNYX_FROM_NUMBER; // e.g. +15551234567 (10DLC or toll-free)
  const profile = process.env.TELNYX_MESSAGING_PROFILE_ID; // optional

  if(!apiKey){
    console.log('[SMS] Telnyx not configured; skipping send to', to, body);
    return { skipped: true };
  }
  if(!from && !profile){
    console.log('[SMS] Need TELNYX_FROM_NUMBER or TELNYX_MESSAGING_PROFILE_ID; skipping');
    return { skipped: true };
  }
