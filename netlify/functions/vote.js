...
exports.handler = async (event) => {
  ...
  try {
    ...
    await base('Votes').create({ 'PlayerPhone': phone, 'Choice': choice, 'Timestamp': new Date().toISOString() })
    return ok({ ok: true })
  } catch (e) { console.error(e); const code=e.response?.status; if(code===401) return bad('Airtable auth failed. Check AIRTABLE_API_KEY / BASE', 500); return bad('Error saving vote', 500) }
}
