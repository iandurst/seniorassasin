...
function NavItem({to, children}) {
  return <NavLink to={to} className={({isActive}) => 'px-3 py-2 rounded-md ' + (isActive ? 'bg-white/10' : 'hover:bg-white/5')}>{children}</NavLink>
}
    ...
    <div className="min-h-screen">
      ...
      <header className="border-b border-white/10 sticky top-0 bg-bg/80 backdrop-blur z-50">
        <div className="container-narrow flex items-center gap-4 py-3">
          ...
          <div className="ml-auto flex items-center gap-2 text-sm text-white/70">{weekLabel()}</div>
