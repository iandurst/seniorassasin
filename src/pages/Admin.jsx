...
function useAdmin() {
  ...
  const include = { credentials: 'include', headers: { 'Content-Type': 'application/json' } }
  return {
    auth(password) {
      return fetch(`${base}/admin-auth`, { method: 'POST', ...include, body: JSON.stringify({ password }) }).then(handle)
    },
    logout() {
      return fetch(`${base}/admin-logout`, { method: 'POST', ...include }).then(handle)
    },
    list() {
      return fetch(`${base}/admin-list`, { ...include }).then(handle)
    },
    verify(id, verified = true) {
      return fetch(`${base}/admin-verify`, { method: 'POST', ...include, body: JSON.stringify({ id, verified }) }).then(handle)
    },
    setAlive(id, alive) {
      return fetch(`${base}/admin-set-alive`, { method: 'POST', ...include, body: JSON.stringify({ id, alive }) }).then(handle)
    },
    eliminate(eliminatorId, eliminatedId) {
      return fetch(`${base}/admin-eliminate`, { method: 'POST', ...include, body: JSON.stringify({ eliminatorId, eliminatedId }) }).then(handle)
    },
    removePlayer(id) {
      return fetch(`${base}/admin-remove-player`, { method: 'POST', ...include, body: JSON.stringify({ id }) }).then(handle)
    },
    resetWeek() {
      return fetch(`${base}/admin-reset-week`, { method: 'POST', ...include }).then(handle)
    },
    resetAll() {
      return fetch(`${base}/admin-reset-all`, { method: 'POST', ...include }).then(handle)
    },
    hardReset() {
      return fetch(`${base}/admin-hard-reset`, { method: 'POST', ...include }).then(handle)
    },
    startWeek() {
      return fetch(`${base}/admin-start-week`, { method: 'POST', ...include }).then(handle)
    },
  }
}
