...
function useAdmin(token) {
  ...
  return {
    auth(password) { return fetch(`${base}/admin-auth`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ password }) }).then(handle) },
...
export default function Admin() {
  ...
  async function unlock() {
    ...
    try {
      ...
      setUnlocked(false); setMsg(null); setError(e.message || 'Incorrect password')
    }
  }

  async function lock() {
    try { await api.logout() } catch {}
    setToken(''); sessionStorage.removeItem('adminToken')
    setUnlocked(false); setTargets([])
    setData({ players: [], prizePool: 0, totalElims: 0 })
  }

  async function refresh() {
    try { const res = await api.list(); setData(res); setError(null) }
    catch (e) { setError(e.message); setUnlocked(false) }
  }

  useEffect(() => { if (token) { setUnlocked(true); refresh() } }, [token])
          ...
          <div className="card p-4">
            <div className="flex gap-2 flex-wrap items-center">
              <button className="btn-ghost" onClick={async()=>{ 
                try { 
                  const res = await api.startWeek(); 
                  await refresh(); 
                  const t = await api.targets(); 
                  setTargets(t.assignments || []); 
                  setMsg(res.warning ? `Week started. ${res.warning}` : 'Week started. Targets loaded.'); 
                  setError(null) 
                } catch(e) { setError(e.message || 'Error starting week') }
              }}>Start Week</button>
